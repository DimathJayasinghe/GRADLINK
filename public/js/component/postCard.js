class PostCard extends HTMLElement {
  ...
  const repostCount = this.getAttribute("repost-count") || "0"; // unused
    ...
    cBtn.addEventListener('click', async () => {
      ...
      if(panel.style.display==='block' && !panel.dataset.loaded){ list.innerHTML='Loading...'; await this._loadComments(postId,list); panel.dataset.loaded='1'; }
    });
    panel.querySelector('.pc-comment-send').addEventListener('click', async ()=>{
      const input=panel.querySelector('.pc-comment-input'); const txt=input.value.trim(); if(!txt) return;
      const fd=new FormData(); fd.append('content',txt);
      input.disabled=true; const r=await fetch(`${window.URLROOT}/post/comment/${postId}`,{method:'POST',body:fd}); const js=await r.json(); input.value=''; input.disabled=false; this._renderComments(js.comments,list); this._setCommentCount(js.comments.length); });

    // Like functionality (client-side demo; replace with API call)
  const likeBtn = this.querySelector('.like-btn');
  likeBtn.addEventListener('click', async ()=>{ const icon=likeBtn.querySelector('i'); const countEl=likeBtn.querySelector('.like-count'); const r=await fetch(`${window.URLROOT}/post/like/${postId}`); const js=await r.json(); const liked=js.status==='liked'; likeBtn.classList.toggle('liked',liked); icon.classList.toggle('fas',liked); icon.classList.toggle('far',!liked); let c=parseInt(countEl.textContent||'0',10); c = liked? c+1: Math.max(0,c-1); countEl.textContent=c; this.setAttribute('like-count',c); });

    // Repost functionality (client-side demo)
  // Repost removed
  }
  async _loadComments(pid,list){ try{ const r=await fetch(`${window.URLROOT}/post/comments/${pid}`); const js=await r.json(); this._renderComments(js,list); this._setCommentCount(js.length);}catch(e){ list.innerHTML='Error'; } }
  _renderComments(arr,list){ if(!Array.isArray(arr)){ list.innerHTML='<em>No comments</em>'; return;} if(!arr.length){ list.innerHTML='<em>No comments</em>'; return;} list.innerHTML=arr.map(c=>`<div style="margin-bottom:10px"><div class="bubble"><strong>${this._esc(c.name||'User')}</strong><br>${this._esc(c.content||'')}<span class="meta">${(c.created_at||'').slice(0,16)}</span></div></div>`).join(''); }
  _esc(s){ return String(s).replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[m])); }
  _setCommentCount(n){ const el=this.querySelector('.comment-count'); if(el) el.textContent=n; }
}
